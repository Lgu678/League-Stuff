...
*/
#include "../../../stdafx.h"
#include "../data_loader/GameDataLoader.h"
#include "../game/Game.h"
...
void Game::runGameLoop()
{
	...
	//START THE MUSIC
	audio->playSound(L"data\\sounds\\introloop.wav", true);
...
void Game::processGameData()
{
	...
	else if (gsm->getCurrentGameState() == GS_LEVEL_SELECT)
	{
		gsm->updateForLevelSelect(this);
	}
...
void Game::quitGame()
{
	...
	TextureManager *worldTextureManager = this->getGraphics()->getWorldTextureManager();
	worldTextureManager->loadTexture(L"data/img/blanktexture.png"); // AN EMPTY TEXTURE
	
	this->getGSM()->getSpriteManager()->clearSprites();

	PoseurSpriteTypesImporter psti;
	psti.loadSpriteTypes(this, SPRITE_TYPES_LIST);
	// CLEAR UP BOX2D WORLD
	// EMPTY RECYCLERS
	...
	audio->stopAllAudio();
	gsm->goToMainMenu();
	this->getGSM()->unloadCurrentLevel();
	this->getGSM()->getPhyiscs()->clearGameWorld();
	this->getGSM()->getPhyiscs()->testResetGameWorld();
	audio->playSound(L"data\\sounds\\introloop.wav", true);
}
