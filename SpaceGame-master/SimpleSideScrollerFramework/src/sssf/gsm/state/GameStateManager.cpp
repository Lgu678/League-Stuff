...
void GameStateManager::addGameRenderItemsToRenderList(Game *game)
{
	...
	if (game->getGSM()->getCurrentGameState() == GS_GAME_IN_PROGRESS)
		world.getLayers()->at(0)->addRenderItemsToRenderList(renderList, viewport);
}
...
void GameStateManager::goToLevelSelectScreen()
{
...
//void GameStateManager::goToVictoryScreen()
//{
//	currentGameState = GS_LEVEL_ONE_VICTORY_SCREEN;
//}

void GameStateManager::goToPauseScreen()
{
...
{
	spriteManager->unloadSprites(this);
	world.unloadWorld();
}
...
void GameStateManager::update(Game *game)
{
	...
	world.update(game);
	if (this->getCurrentGameState() == GS_GAME_IN_PROGRESS)
	{
		physics.gameWorldStep();
	}
}

void GameStateManager::updateForLevelSelect(Game *game)
{
	spriteManager->updateShootingStars(game);
	physics.levelSelectWorldStep();
