...
#include "../input/ButtonEventHandler.h"
...
void Button::initButton(int initNormalTextureID, int mouseOverTransitions[], int sizeOfArr, int initX, int initY, int initZ, int initAlpha, int initWidth, int initHeight, bool initMouseOver, wstring initCommand)
{
	normalTextureID = initNormalTextureID;
	x = initX;
	y = initY;
	...
	transitionNumbers = sizeOfArr;

	dynamicMouseOverTextureID = new int[sizeOfArr];

	for (int i = 0; i < sizeOfArr; i++)
	{
			dynamicMouseOverTextureID[i] = *(mouseOverTransitions + i);
	}
}
...
void Button::nextTransition()
{
	transition += transController;

	if (transition >= transitionNumbers)
	{
		transController = -1;
		transition = transitionNumbers - 1;
	}
...
void Button::decTransitionCountdown()
{
	transitionCountdown--;
